...
(function(){
  ...
  window.triggerFromage=function(){
    ...
    const endT=setTimeout(()=>{timers=[];running=false;},DURATION+600);timers.push(endT);
  };
})();
...
(function(){
  ...
  window.triggerGay=function(){
    if(running)return;running=true;injectStyles();
    const overlay=document.createElement('div');overlay.className='gay-overlay';overlay.style.setProperty('--gTotal',DURATION+'ms');document.body.appendChild(overlay);
    ...
    const endT=setTimeout(()=>{overlay.remove();timers=[];running=false;},DURATION+600);timers.push(endT);
  };
})();
